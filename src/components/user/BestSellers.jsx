...
export default function BestSellers({ initialData }) {
  ...
  useEffect(() => {
    ...
    fetchHomeData()
      .then(d => { if (d?.bestSellers) setProducts(d.bestSellers) })
      .catch(() =>
        fetch(`${BASE}product?isBestSeller=true&limit=8`)
    ...
    <section className="relative py-14 px-4 md:px-8 overflow-hidden"
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        <div className="flex items-end justify-between mb-3">
          <div>
            <span className="text-xs font-bold tracking-[0.3em] uppercase font-sans block mb-2" style={{ color: '#E05C88' }}>CUSTOMER FAVOURITES</span>
            <h2 className="text-3xl md:text-5xl font-bold font-serif leading-tight" style={{ color: '#7B2447' }}>Best Sellers</h2>
          </div>
          <Link href="/shop?filter=isBestSeller" className="group font-semibold text-sm flex items-center gap-1 ml-4 pb-1" style={{ color: '#E05C88' }}>
