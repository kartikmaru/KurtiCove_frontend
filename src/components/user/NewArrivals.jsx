  ...
  return (
    <div className="group relative bg-white rounded-3xl overflow-hidden cursor-pointer transition-all duration-500 hover:-translate-y-2 shadow-card hover:shadow-card-hover border"
        ...
        <div className="absolute inset-0" style={{ background: '#FEF0E3' }}>
          {img1 && (
            ...
            <img src={img1} alt={product.name}
              className="absolute inset-0 w-full h-full object-contain transition-all duration-700 group-hover:opacity-0 group-hover:scale-105" />
          )}
          {img2 && (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={img2} alt={`${product.name} alt`}
              className="absolute inset-0 w-full h-full object-contain transition-all duration-700 opacity-0 group-hover:opacity-100 group-hover:scale-100 scale-105" />
          )}
        </div>
        {/* NEW badge — mint */}
      ...
      <div className="p-5" style={{ background: 'linear-gradient(to bottom, #fff, #FEF0E3)' }}>
        <p className="font-serif font-semibold text-base leading-snug mb-2 line-clamp-1" style={{ color: '#7B2447' }}>
          {product.name}
        </p>
        {product.description && (
          <p className="text-xs font-sans leading-snug mb-2.5" style={{ color: '#6B4553', display:'-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical', overflow:'hidden' }}>
            {product.description}{' '}
            <Link href={`/product/${product._id}#description`} onClick={e => e.stopPropagation()}
              className="underline underline-offset-2 font-medium" style={{ color: '#E05C88' }}>More</Link>
          </p>
        )}
      ...
      <div className="flex justify-center gap-2 mt-4">
        {products.map((_, i) => (
          <button key={i} onClick={() => { const el = cardRefs.current[i]; if (el) el.scrollIntoView({ behavior:'smooth', block:'nearest', inline:'center' }) }}
            ...
            style={{ width: i === activeIdx ? 24 : 10, height: i === activeIdx ? 10 : 10, background: i === activeIdx ? '#E05C88' : '#F8A5B5' }} />
        ))}
      </div>
    </div>
  )
...
export default function NewArrivals() {
  ...
  useEffect(() => {
    fetch(`${BASE}product?isNewArrival=true&limit=8`, { cache: 'no-store' })
  ...
  return (
    <section className="relative py-16 px-4 md:px-8 overflow-hidden"
      style={{ backgroundImage:'url(/bg%20image/newarrival.jpg)', backgroundSize:'cover', backgroundPosition:'center top' }}>
      <div className="absolute inset-0 pointer-events-none" style={{ background: 'rgba(252,250,224,0.88)' }} aria-hidden="true" />
      <div className="relative z-10 max-w-7xl mx-auto">
        <div className="flex items-end justify-between mb-3">
          <div>
            <span className="text-xs font-bold tracking-[0.3em] uppercase font-sans block mb-2" style={{ color: '#E05C88' }}>FRESH FROM THE LOOM</span>
            <h2 className="text-4xl md:text-5xl font-bold font-serif leading-tight" style={{ color: '#7B2447' }}>New Arrivals</h2>
          </div>
          <span className="group flex items-center gap-1 ml-4 pb-1">
            <Link href="/shop?filter=isNewArrival" className="font-semibold text-sm flex items-center gap-1" style={{ color: '#E05C88' }}>
        ...
        {loading ? (
          <>
            ...
            <div className="hidden md:grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 md:gap-8">{Array.from({length:4}).map((_,i) => <SkeletonCard key={i} />)}</div>
          </>
        ) : products.length === 0 ? (
          <p className="text-center text-lg font-sans py-16" style={{ color: '#6B4553' }}>No new arrivals right now. Check back soon!</p>
        ) : (
          <>
