...
export default function CartPage() {
  ...
  /* Badge / heading count = unique line items, not total quantity */
  ...
  const handleQty = (id, flag) => {
    ...
    if (item) {
      const newQty = flag === 'inc' ? item.qty + 1 : item.qty - 1
      if (newQty > 0) {
  ...
  return (
    /* White background — consistent with home / shop / details */
    <main className="min-h-screen py-10 bg-white">
      <div className="max-w-5xl mx-auto px-4 sm:px-6">

        {/* Heading — font-sans (Poppins), not font-cursive / font-serif */}
        <h1 className="font-sans text-3xl font-bold mb-8" style={{ color: BERRY }}>
          Your Cart
          {uniqueCount > 0 && (
        ...
        {items.length === 0 ? (
          <div className="bg-white rounded-[16px] p-16 text-center shadow-card"
            ...
            <ShoppingCart size={52} style={{ color: BORDER }} className="mx-auto mb-4" />
            <h2 className="font-sans text-2xl font-bold mb-2" style={{ color: BERRY }}>Your cart is empty</h2>
              ...
              {items.map((item) => (
                <div key={item._id}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {/* Price — font-sans throughout, no font-serif */}
                    <div className="flex items-center gap-2 mt-1">
                      <span className="font-sans font-bold text-sm" style={{ color: ROSE }}>
                    ...
                    <div className="flex items-center justify-between mt-3">
                      <div className="flex items-center border rounded-lg overflow-hidden" style={{ borderColor: BORDER }}>
                        <button onClick={() => handleQty(item._id, 'dec')}
                          className="px-2.5 py-1.5 transition-colors" style={{ color: BERRY }}>
                          <Minus size={13} />
                        </button>
                        ...
                        <button onClick={() => handleQty(item._id, 'inc')}
                          className="px-2.5 py-1.5 transition-colors" style={{ color: BERRY }}>
                          <Plus size={13} />
                        </button>
                      </div>
                  ...
                  {/* Line total — font-sans */}
                  <div className="text-right flex-shrink-0">
                    <p className="font-sans font-bold text-sm" style={{ color: BERRY }}>
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-[16px] p-6 shadow-card sticky top-20"
                ...
                <div className="space-y-3 mb-5">
                  ...
                  {/* Total amount — font-sans, not font-serif */}
                  <div className="flex justify-between font-sans font-bold" style={{ color: BERRY }}>
                    <span>Total</span>
                    <span className="font-sans text-lg">₹{grandTotal.toLocaleString()}</span>
                  </div>
                </div>
